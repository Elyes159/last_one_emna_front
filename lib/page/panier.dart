...
class PanierPage extends StatefulWidget {
  ...
  const PanierPage({Key? key, required this.id, required this.quantite})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    if (cartProducts.isNotEmpty) {
      for (int i = 0; i < cartProducts.length; i++) {
        totalMontantHT +=
            (cartProducts[i]["productDetails"]['Montant HT'] ?? 0) *
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Transform.translate(
                  offset: Offset(-150,
                      -25), // Ajuster la marge en haut et à gauche de l'icône
                  child: IconButton(
                    icon: Icon(Icons.arrow_back,
                        color: Colors
                            .black), // Utilisation de l'IconButton avec l'icône de retour
                    onPressed: () {
                        ...
                        child: Row(
                          ...
                          children: [
                            Transform.translate(
                              offset: Offset(1, -4),
                              child: Image.network(
                                cartProducts[index]["productDetails"]
                                        ['Image'] ??
                                    "",
                                width: 110,
                                height: 900,
                                ...
                                child: RichText(
                                  text: TextSpan(
                                    children: [
                                      ...
                                      TextSpan(
                                        text:
                                            '$totalMontantHT', // Prix actuel du produit
                                        style: TextStyle(
                                          color: Color(0xFF006E7F),
                                          fontSize: 29,
                                          fontWeight: FontWeight.bold,
...
Future<void> deleteCartProduct(String userId, String itemId) async {
  try {
    final response = await http.delete(
      Uri.parse(
          "http://192.168.1.17:3003/user/delete-cart-item/$userId/$itemId"),
    );
    if (response.statusCode == 200) {
      print("Effacé avec succès");
    } else {
      print(
          'Erreur lors de la suppression de l\'article du panier: ${response.body}');
    }
  } catch (e) {
